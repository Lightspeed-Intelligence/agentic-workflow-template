...
- 准备脚本自身限时 13 分钟，步骤级 `timeout-minutes: 15` 作为更宽的兜底。之所以由脚本
  自己限时：步骤级超时会让 runner 直接杀掉进程树并判定步骤失败，降级逻辑没有机会运行。
  脚本内限时则把超时变成普通的非零退出码，和执行失败走同一条披露路径。超时后已写入的
  `PATH`、环境变量和已装好的部分依赖仍然保留，Agent 会收到「准备未完成」的说明。
