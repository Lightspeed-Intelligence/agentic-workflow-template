...
前提：可报告范围始终限于**本 PR 引入或放大**的问题。仅被本 PR 触及或依赖、但未被其放大的旧缺陷不在报告范围内（见 review-sop 的 Finding Discipline）。在够格报告的前提下,按根因归类来源：

- **本轮改动引入**: 问题代码行由本轮审查范围内的 commit 写入（落在 workflow 提供的审查范围 diff 内）；或本轮改动使原本可接受的旧代码变坏的交互型问题——即缺陷由本轮**放大**造成（此时虽 blame 可能指向旧代码，缺陷仍由本轮引入，归此类）。判定带语义，不是纯机械 blame。
- **既存(非本轮引入)**: 缺陷根因在旧代码、`git blame` 指向本轮审查范围之外，且**已被本 PR 放大**而够格报告——本 PR 放大了它的影响,但根子是老的。用于让作者知道这是旧债被本次触发,而非本次改出。不拖入与本 PR 完全无关、未被放大的祖传问题。
