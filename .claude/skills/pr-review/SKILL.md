...
- 每条 finding 必须用 `git blame` 判定来源并在正文标注：问题代码落在本轮审查范围内 → 「本轮改动引入」；早于本轮范围且本 PR 只是触及/依赖/放大它 → 「既存(非本轮引入)」；本轮改动使旧代码变坏的交互型问题归「本轮改动引入」。此溯源是无状态的本地计算（基于 workflow 提供的审查范围与 `git blame`），不依赖历史评论或 marker，首轮审查同样适用，与「历史问题复核」是两个独立维度。来源不改变严重度、阻塞与计数，既存问题按真实严重度照常处理，仅供作者区分本次改出的问题与本次撞上的旧债。
