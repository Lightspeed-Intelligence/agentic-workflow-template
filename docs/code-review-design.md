...
Agent 有完整本地权限，但 runner 只预装通用工具链。需要特定 JDK 版本、Python 依赖或
预构建上游产物的项目，可以通过 `setup_script` 声明一个准备脚本。审查器因此能运行项目
自身的编译与测试，捕获「声明了输出但没有下游读取」「配置缺少必需键」这类只在编译期
暴露的问题。

脚本从 PR 的 base commit 读取，当前 PR 无法修改它。但它读取的数据来自 head 工作树，
`pip install -r requirements.txt` 会执行依赖包的 `setup.py`，所以这个来源限制只阻断
一类直接注入，不是安全边界。审查器本来就会执行 head 里的项目代码，增加准备脚本没有
跨越新的边界；真正的边界仍是只读 job token 与不向 Agent 进程注入 GitHub 凭据。

准备脚本不接收任何 secret，执行失败也不终止审查——退出码和日志末尾作为不可信数据
交给 Agent，由它在评论里说明哪些验证没能进行。这与「单个项目测试因缺少工具而未运行
应记录但不单独使审查失败」是同一条约定。
